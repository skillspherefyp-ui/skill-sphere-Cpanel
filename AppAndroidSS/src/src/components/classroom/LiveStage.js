...
export function StageCaption({ text, progress = 1, color = '#a5b4fc', playing = true, windowWords = 0, size = 16 }) {
  ...
  const maxChars = containerWidth > 0 ? Math.floor(containerWidth / charWidth) : 999;

  // Walk backwards from `shown` collecting words until we'd exceed maxChars
  let charCount = 0;
  let startIdx = shown;
  for (let i = shown - 1; i >= 0; i--) {
    const needed = words[i].length + (charCount > 0 ? 1 : 0);
    if (charCount + needed > maxChars) break;
    charCount += needed;
    startIdx = i;
  }
  const visible = words.slice(startIdx, shown).join(' ');
  ...
  return (
    <Text
      ...
      style={[
        captionStyles.text,
        { fontSize: size, lineHeight: Math.round(size * 1.5) },
