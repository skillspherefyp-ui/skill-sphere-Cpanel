...
const CourseCard = ({
  ...
  const getCourseTypeBadge = () => {
    ...
    return (
      <View style={[styles.courseTypeBadge, { backgroundColor: isAI ? 'rgba(139,92,246,0.12)' : 'rgba(34,211,238,0.1)', borderColor: isAI ? 'rgba(139,92,246,0.3)' : 'rgba(34,211,238,0.25)' }]}>
        <Icon name={isAI ? 'sparkles' : 'person'} size={11} color={isAI ? '#8B5CF6' : '#22D3EE'} />
        <Text style={[styles.courseTypeBadgeText, { color: isAI ? '#8B5CF6' : '#22D3EE' }]}>
          {isAI ? 'AI-Powered' : 'Instructor-Led'}
        </Text>
      </View>
    );
  };
